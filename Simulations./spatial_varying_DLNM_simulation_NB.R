...
for (i in 1:nsim){
  ...
  suppressWarnings(y_all <- rnbinom(length(x), size = 5, mu = offset*exp(-10.5+cumeff+random_area))) 
  ...
  for (c in 1:4){
    tryCatch({
      ...
      model_laplace[[c]] <- DLNM_Laplace(y_all ~ 1,
                                         ...
                                         pen_crossbasis = pen_crossbasis_list[[c]], DIC = T, CPO = T,
                                         WAIC = T, family = "NB") 
    },error = function(e){
  ...
  if(succes_flags){  
    ...
    ind_max_CPO = which.max(c(sum(model_laplace[[1]]$CPO), sum(model_laplace[[2]]$CPO),
                              sum(model_laplace[[3]]$CPO), sum(model_laplace[[4]]$CPO),
                              sum(model_laplace[[5]]$CPO)))
