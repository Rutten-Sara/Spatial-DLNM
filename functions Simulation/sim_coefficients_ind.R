# Simulate region-specific coefficients for simulation surface (strong heterogeneity)
simulate_coefficients <- function(n_regions, coef_base, map, seed = 3, rho = NULL){
  p <- length(coef_base)
    ...
    Sigma_coef <- c(0.01,0.05,0.005,0.005,0.001)
    ...
    for (i in 1:5){
      ...
      coefficients_all[[i]][,1] <- 1.5*coef_base[i]*U_mat_i
